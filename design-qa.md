...
- Published red-theme capture: `qa-red-viewport.png`
- State: 蓝色通用与红色母婴主题；海淀店选中，朝阳店未选中。
...
### Red-theme published-page verification

`qa-red-viewport.png` was captured from `https://guiping02-byte.github.io/ui-guidelines/?v=f243c5f`. It shows the same full-width component in the red board with the selected card using the editable red brand color and pale red surface.
...
- Colors and visual tokens: blue selected border/native radio use `#2C89FF` with `#F5FAFF` background. Red selected border/native radio resolve from `var(--brand)` (default `#D62F36`) with `var(--brand-pale)` background (default rendered `#FDF3F3`). Unselected border uses `#E5E6EB`; title and detail colors use `#222222` and `#646464`.
...
- The block is visible in both themes and adopts each theme's semantic tokens without duplicating the component markup.
...
- [x] Blue and red theme token mapping
...
4. Published-page verification confirmed the red selected state resolves to `rgb(214, 47, 54)` border, `rgb(253, 243, 243)` background, and exactly one checked radio after switching stores.
